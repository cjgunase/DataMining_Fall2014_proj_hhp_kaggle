...
library(earth)
marsFit <- earth(x = alldata[trainrows,-targindex], y = alldata[trainrows,targindex])
prediction<-predict(marsFit,newdata = alldata[scorerows,-targindex])

train <-as.data.frame(alldata[trainrows,])
test <-as.data.frame(alldata[scorerows,])
lmFit <-lm(train$DaysInHospital~.,data=train)
...
robust<-train(train[,-2], train[,2],method = "rlm")
